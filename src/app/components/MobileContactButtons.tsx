...
const MobileContactButtons = () => {
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-white shadow-lg flex items-center md:hidden text-xs">
      ...
      {/* Botão "Fale por WhatsApp" */}
      <motion.div
        whileHover={{ scale: 1.05 }}
        whileTap={{ scale: 0.95 }}
        className="w-full bg-[#25D366] text-white font-semibold text-center  py-3 flex items-center justify-center gap-2"
      >
        <Link
          href="https://wa.me/5561998570759" // Substitua pelo número correto
          className="flex items-center justify-center gap-2 w-full h-full"
        >
          <svg
            className="h-5 w-5"
